...
parser.add_argument('--batch_size', default=12, type=int, help='train batchsize')
parser.add_argument('--lr', '--learning_rate', default=0.0001, type=float, help='initial learning rate') # follow the Clothing1M dataset.
parser.add_argument('-lr_decay_rate', type=float, default=0.1, help='decay rate for learning rate')
# parser.add_argument('--lr_decay_epochs', type=str, default='60,80', help='decay epochs for learning rate')
parser.add_argument('--cosine', action='store_true', default=False,
                    help='use cosine lr schedule')
parser.add_argument('--noise_path', type=str, help='path of chaoyang noise file.', default=None)
...
parser.add_argument('--dataset', default='chaoyang', type=str)
...
# TODO: for the hyperparameters please refer to the paper and large dataset.
parser.add_argument('--tau', default=0.99, type=float,
                    help='high-confidence selection threshold')
parser.add_argument('--pretrain_ep', default=5, type=int, help = 'warm-up training epoch') # TODO: what's the difference between pretrain_ep and warmup_ep?
parser.add_argument('--warmup_ep', default=5, type=int, help = 'parameter ramp-up epoch')
parser.add_argument('--low_conf_del', action='store_true', default=False) # TODO: what's this?
parser.add_argument('--threshold', default=0.7, type=float, help = 'threshold of label guessing') # Label Guessing Agreement (LGA) threshold. 
parser.add_argument('--fmix', action='store_true', default=False)
parser.add_argument('--start_expand', default=40, type=int)       # the K value in the paper.        
parser.add_argument('--debias_output', default=0.8, type=float,   # alpha in the paper.
...
parser.add_argument('--no_annotator', default='label',choices=['label_A', 'label_B','label_C', 'label'], type=str, help='number of annotators')
...
# Hyper Parameters
running_name = 'Baseline_Chaoyang_Promix_' + args.dataset + '_' + args.no_annotator
...
if args.data_path is None:
    if args.dataset == 'chaoyang':
        args.data_path = '/chaoyang'
    else:
        pass

if args.dataset == 'chaoyang':
    args.noise_path = 'chaoyang/json/'
...
def train(epoch, net, net2, optimizer, labeled_trainloader, pi1, pi2, pi1_unrel, pi2_unrel): 
    ...
    # img1, img2, target, prob
    # why the code has the w_x and w_x2?
    # img1, img2, target, prob, prob2, index
    for batch_idx, (inputs_x, inputs_x2, labels_x, w_x, w_x2, index) in tqdm(enumerate(labeled_trainloader)):
        with autocast(dtype=torch.float16):
            ...
            # ipdb.set_trace()
            # why the X_w_c is with the shape of [12, 3, 224, 224]?
            x_fmix = fmix(X_w_c) # what's the x_fmix?
        ...
        if batch_idx % 20 == 0 :
            print('%s:%s | Epoch [%3d/%3d] Iter[%3d/%3d]\t Net1 loss: %.2f  Net2 loss: %.2f'
                         % (args.dataset, args.no_annotator, epoch, args.num_epochs, batch_idx + 1, num_iter,
                            loss_net1.item(), loss_net2.item()))

    return pi1,pi2,pi1_unrel,pi2_unrel


def warmup(epoch, net, net2, optimizer, dataloader): # just warmup one network.
    ...
    for batch_idx, (inputs_w, labels, _) in enumerate(dataloader): # img, target, index
        ...
        #     L = loss + penalty
        # else:
        # FIXME: whether the penalty is necessary for the chaoyang dataset?
        L = loss # for the chaoyang dataset, we could just use the loss.
        L.backward()
        optimizer.step()
        ...
        if batch_idx % 20 == 0:
            print('%s:%s | Epoch [%3d/%3d] Iter[%3d/%3d]\t CE-loss: %.4f  Penalty-loss: %.4f  All-loss: %.4f'
                         % (
                         args.dataset, args.no_annotator, epoch, args.num_epochs, batch_idx + 1, num_iter,loss.item(),penalty.item(), L.item()))


        wandb.log({'CE-loss': l_ce.item(), 'Penalty-loss': penalty.item(), 'All-loss': L.item()}) if args.wandb else None

    # inputs_w, labels, _ = next(iter(dataloader))
    # inputs_w, labels = inputs_w.cuda(), labels.cuda()
    # optimizer.zero_grad()
    # outputs = net(inputs_w) 
    # outputs2 = net2(inputs_w)
...
def evaluate(loader, model, save = False, best_acc = 0.0):
    ...
    if save:
        if acc > best_acc:
            ...
            save_path= os.path.join('./', args.no_annotator +'best.pth.tar')
...
def test(epoch, net1, net2):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in tqdm(enumerate(test_loader)):
            ...
            outputs_mean_ori_softmax = (F.softmax(outputs1_ori, dim=1) + F.softmax(outputs2_ori, dim=1)) / 2
...
def create_model():
    model = DualNet2(args.num_class)
    model = model.cuda()
    return model


stats_log = open(os.path.join(save_dir, '%s_%s_anno_%ss' % (args.dataset, args.num_epochs, args.no_annotator) + '_stats.txt'), 'w')
test_log = open(os.path.join(save_dir, '%s_%s_anno_%ss' % (args.dataset, args.num_epochs, args.no_annotator) + '_acc.txt'), 'w')

warm_up = args.pretrain_ep
#unique file name to record the synthetic noise for CIFAR-10/100
time = str(datetime.now())[-6:]

loader = dataloader.chaoyang_dataloader(root=args.data_path,
                                        batch_size=args.batch_size,
                                        num_workers=4,
                                        annotator=args.no_annotator)
